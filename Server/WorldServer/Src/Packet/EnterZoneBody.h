...
namespace World
{
    ...
    //
    // **바이트 예산으로 잘라낸다.** 우편함 통 수 상한(Protocol::kMaxMailCount)만으로는
        ...
        const std::unordered_map<uint8_t, int64_t>& currencies)
    {
        ...
        std::vector<const MailInfo*> sorted;
        ...
        std::sort(sorted.begin(), sorted.end(),
                  [](const MailInfo* left, const MailInfo* right) { return right->mailId < left->mailId; });

        std::vector<const MailInfo*> selected;
        selected.reserve(sorted.size());
        size_t usedBytes = 0;
        for (const auto* const info : sorted)
        {
            const size_t entryBytes = sizeof(Protocol::MailId::ValueType)

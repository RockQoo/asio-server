...
BasicProcessor::BasicProcessor(PlayerManager::Mutexed& playerManager, ZoneLinkRegistry::Mutexed& zoneLinkRegistry,
...
// AddProcessor가 기동 때 한 번 부른다. **레인 사이 메시지**를 여기 등록하고, 그 안에서
// 실제 클라/존 패킷을 어떻게 나눌지는 아래 Dispatcher 두 개가 맡는다.
void BasicProcessor::RegistHandler()
{
    Regist(EWorldMsg::OnRecvStream,     &BasicProcessor::OnRecvStream);
    Regist(EWorldMsg::FromClientStream, &BasicProcessor::OnRecvFromClientStream);

    Regist(EWorldMsg::OnRecvZoneStream, &BasicProcessor::OnRecvZoneStream);
    Regist(EWorldMsg::FromZoneStream,   &BasicProcessor::OnFromZoneStream);
    Regist(EWorldMsg::OnZoneLinkClosed, &BasicProcessor::OnZoneLinkClosed);

    Regist(EWorldMsg::BroadcastToAll,   &BasicProcessor::OnBroadcastToAll);

    RegisterZoneOwnerIds();
    RegisterPackets();
}
...
void BasicProcessor::OnRecvStream(const Pipeline::OwnerId& /*owner*/, const RecvStreamBody& body)
{
    ...
    const auto clientSessionId = Packet::PeekOwnerId<Network::SessionId>(body.payload);
    if (!clientSessionId)
    {
        LOG.Warning(ELogCategory::Gateway, "ownerId를 읽을 수 없는 패킷, 버림")
            .KV("PacketId", static_cast<uint16_t>(body.packetId))
            .KV("PayloadSize", body.payload.size());
        return;
    }
...
// ── ③ 콘텐츠 ────────────────────────────────────────────────────────────────
// **owner = clientSessionId.** 이후 콘텐츠 핸들러가 이 스레드에서 돈다.
void BasicProcessor::OnRecvFromClientStream(const Pipeline::OwnerId& /*owner*/, const RecvStreamBody& body)
{
    DispatchFromGateway(body.packetId, body.session, body.payload);
}

// 존 링크도 같은 2단이다. **owner = 존 세션 id.**
void BasicProcessor::OnRecvZoneStream(const Pipeline::OwnerId& /*owner*/, const RecvStreamBody& body)
{
    const auto ownerId = zoneOwnerIds_.Find(body.packetId, body.payload);
    if (!ownerId)
    {
...
void BasicProcessor::OnFromZoneStream(const Pipeline::OwnerId& /*owner*/, const RecvStreamBody& body)
{
    DispatchFromZone(body.packetId, body.session, body.payload);
}

void BasicProcessor::OnZoneLinkClosed(const Pipeline::OwnerId& owner)
{
    RemoveZoneLink(static_cast<Network::SessionId>(owner.value));
}

void BasicProcessor::RegisterPackets()
{
    gatewayDispatcher_.Register(this, &BasicProcessor::HandleClientConnected);
    gatewayDispatcher_.Register(this, &BasicProcessor::HandleClientDisconnected);
    gatewayDispatcher_.Register(this, &BasicProcessor::HandleFromClient);

    zoneDispatcher_.Register(this, &BasicProcessor::HandleZoneRegister);
    zoneDispatcher_.Register(this, &BasicProcessor::HandleForwardToWorld);
    zoneDispatcher_.Register(this, &BasicProcessor::HandleZoneTransfer);
    zoneDispatcher_.Register(this, &BasicProcessor::HandleUnitOfWorkStream);
}

void BasicProcessor::DispatchFromGateway(const PacketId packetId, const Network::Session::SPtr& gatewaySession,
                                        const std::span<const byte> payload)
{
    gatewayDispatcher_.Dispatch(packetId, gatewaySession, payload);
}

void BasicProcessor::DispatchFromZone(const PacketId packetId, const Network::Session::SPtr& zoneSession,
                                     const std::span<const byte> payload)
{
    zoneDispatcher_.Dispatch(packetId, zoneSession, payload);
}

void BasicProcessor::RemoveZoneLink(const Network::SessionId zoneSessionId)
{
...
void BasicProcessor::HandleClientConnected(const Network::Session::SPtr& gatewaySession,
...
void BasicProcessor::HandleClientDisconnected(const Network::Session::SPtr& /*gatewaySession*/,
...
void BasicProcessor::HandleFromClient(const Network::Session::SPtr& gatewaySession,
...
void BasicProcessor::HandleForwardToWorld(const Network::Session::SPtr& /*zoneSession*/,
...
void BasicProcessor::HandleZoneTransfer(const Network::Session::SPtr& /*zoneSession*/,
...
std::vector<byte> BasicProcessor::EnterZoneBodyFor(const Common::W2ZEnterZoneHead& enterZone) const
{
...
void BasicProcessor::ReturnToSourceZone(Common::Z2WZoneTransfer transfer) const
{
...
void BasicProcessor::HandleUnitOfWorkStream(const Network::Session::SPtr& /*zoneSession*/,
                                           const Common::Z2WUnitOfWorkStream& packet)
{
    ...
    for (const auto& task : packet.tasks)
    {
        ...
        // (Server/Common/Src/TaskKind.h) 여기서 2단으로 분기한다. 콘텐츠가 늘면
...
void BasicProcessor::BroadcastToAll(const PacketId clientPacketId, const std::span<const byte> payload)
{
    ...
    // 막을 수 없다.
    // **주인 없는 경로를 만들지 않는다** -- 고정 상수를 주인으로 줘서 이 일을 한 레인에
    // 직렬화한다. 전체 순회라는 무거운 일이 한 스레드에 쌓이지만, 대신 "주인 없이 도는
    // 메시지"라는 예외가 구조에 생기지 않는다.
    RecvStreamBody body{};
    body.packetId = clientPacketId;
    body.payload.assign(payload.begin(), payload.end());

    Pipeline::PushMsg<Pipeline::EProducerType::Basic>(
        EWorldMsg::BroadcastToAll, GetProcessorId(), Pipeline::OwnerId{kGlobalQueryKey}, std::move(body));
}

void BasicProcessor::OnBroadcastToAll(const Pipeline::OwnerId& /*owner*/, const RecvStreamBody& body)
{
    const auto clientPacketId = body.packetId;
    const auto& payloadCopy = body.payload;
    {
        {
            size_t sentCount = 0;

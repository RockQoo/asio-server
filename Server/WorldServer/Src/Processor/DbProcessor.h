...
// DB 레인(EWorldProcessorId::Db)에서 도는 일을 맡는 프로세서.
//
...
{
public:
    DbProcessor(DbConnectionPool& dbPool, Processor::Group<EWorldProcessorId>& dbGroup);
    ...
    {
        dbGroup_.Post(EWorldProcessorId::Db, ownerId, std::forward<F>(work));
    }

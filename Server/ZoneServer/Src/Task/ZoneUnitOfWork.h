...
//
// **UoW 의 주인은 playerId 다.** 이 값은 직렬화돼 World 로 건너가고, World 는 그 값으로
// 자기 캐시를 찾고 자기 레인을 고른다 -- 그래서 존과 World 가 같은 주인을 쓴다. 세션 id 를
// 쓰면 재접속마다 주인이 바뀌어서, 같은 사람의 변경이 접속할 때마다 다른 레인으로 간다.
//

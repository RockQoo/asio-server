...
namespace Zone
{
    ...
    UnitOfWork::UnitOfWork(WorldLink& worldLink, Player& player, const PacketId requestPacketId)
    ...
    void UnitOfWork::RollbackAll() noexcept
    {
        ...
        catch (const std::exception& ex)
        {
            ...
            LOG.Error(ELogCategory::Zone, "UnitOfWork 롤백 중 예외 -- 메모리와 DB가 어긋난다")
                .KV("ClientSessionId", clientSessionId_).KV("What", ex.what());
        }
        catch (...)
        {
            LOG.Error(ELogCategory::Zone, "UnitOfWork 롤백 중 알 수 없는 예외")
                .KV("ClientSessionId", clientSessionId_);
        }
    ...
    void UnitOfWork::SendToWorld(const std::span<const byte> stream) const
    {

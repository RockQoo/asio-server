...
public:
    ...
    [[nodiscard]] std::shared_ptr<MailModel::Mutexed> Add(const Network::SessionId clientSessionId,
                                                      const Common::PlayerId playerId,
                                                      std::vector<Common::MailInfo> initial);
    void Remove(const Network::SessionId clientSessionId);

    [[nodiscard]] std::shared_ptr<MailModel::Mutexed> Find(const Network::SessionId clientSessionId) const;

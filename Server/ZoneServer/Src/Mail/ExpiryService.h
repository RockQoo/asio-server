...
namespace Mail
{
    ...
    // 지점이다: 이 스윕과 존 로직 스레드의 AddMail/DelMail(ZoneUnitOfWork 경유)이 같은 Model
    // 인스턴스를 동시에 건드릴 수 있기 때문이다. 나머지 게임 상태(PlayerState 등)는 여전히
    // 존 로직 스레드 전용이라 락이 없다는 것과 대비된다.
    class ExpiryService
    {
    public:
        ExpiryService(Registry& mailRegistry, Network::SessionHolder& worldLink);

        void SweepOnce(const int64_t nowUt);

    private:
        Registry& mailRegistry_;

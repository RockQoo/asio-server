...
namespace Mail
{
    Model::Model(std::vector<Common::MailInfo> initial)
    {
    ...
    EErrorCode Model::AddMail(Common::MailInfo info, Task::UnitOfWork& unitOfWork)
    {
    ...
    EErrorCode Model::InsertMail(Common::MailInfo info, Task::UnitOfWork& unitOfWork)
    {
        const auto mailId = info.mailId;
        if (mails_.contains(mailId))
        {
    ...
    EErrorCode Model::DelMail(const Common::MailId mailId, Task::UnitOfWork& unitOfWork, const bool /*isTimeout*/)
    {
    ...
    std::vector<Common::MailId> Model::TakeExpiredMailIds(const int64_t nowUt) const
    {

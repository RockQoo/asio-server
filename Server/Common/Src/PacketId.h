...
namespace Common
{
    ...
    //
    // 방향마다 1000 단위로 대역을 잘라 쓴다. Header에는 id 하나뿐이라 헤더만 봐서는
    ...
    enum class PacketId : uint16_t
    {
        ...
        // (BasicProcessor::HandleFromClient -> LoginProcessor).
...
// `ELogCategory`(Server/Core/Src/Log/LogCategory.h)와 같은 방식이다. 같은 이유로 전역에

...
이력이라 갱신이 늦고, 그러면 요약본이 원문과 갈려서 거짓이 된다(실제로 그랬다).

## 3. 다음에 하면 좋을 일

1. **성능 개선** — 2026-09-18 측정으로 기준선과 우선순위가 정해졌다.
   수치와 판단 근거는 [`docs/performance.html`](docs/performance.html).
   ...
   **측정 시나리오는 고정이다** — 바꾸면 이전 회차와 비교가 깨진다. A(DB) · B(브로드캐스트) ·
   C(한계)를 매번 같은 인자로 돌리고, 결과는 `docs/performance.html` 의 이력 표에 한 행씩
   쌓는다.
...
2. **남은 DB 연동**: 스키마와 계층은 준비됐고, Zone의 `UnitOfWork` 태스크를 실제 SP로
   ...
   같이 정할 것: 게임 스키마와 공유 저장 프로시저는 저장소 루트의 `Sql/`에 둔다 — `Tool/` 아래

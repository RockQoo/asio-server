...
### 2026-09-15에 추가된 것 — 전투(근접/원거리), 유닛, 틱 콘텐츠

- **전투를 존 레인에 올렸다**: 근접/원거리 각 1종, 더미 몬스터 3기/존, 사망 + 8초 리스폰,
  "사거리 안 가장 가까운 플레이어를 때린다"짜리 몬스터 AI. 스탯은 HP/MP/ATK/DEF뿐이고
  크리티컬·회피·속성·어그로 테이블·드랍·경험치는 넣지 않았다.
  **DB 스키마를 한 줄도 건드리지 않는다** — HP는 영속 대상이 아니라 존 메모리에만 있다.
- **이게 이 저장소에서 틱이 처음 생기는 콘텐츠다.** 로그인·이동·우편은 전부 요청–응답이라
  주인을 "보낸 사람"으로 잡으면 끝났는데, 전투는 아무도 요청하지 않아도 서버가 혼자 시간을
  굴리고(리젠·리스폰·AI) **그 시간이 만지는 데이터를 패킷도 만진다.**
  그래서 **`C2ZAttack`만 클라이언트 요청 중 유일하게 레인을 갈아탄다** — 플레이어 레인이
  파싱만 하고 `PostToZone(zoneId)`로 다시 던진다. 틱과 같은 strand에 서므로
  `Combat::` 전체에 **뮤텍스가 0개**다. 근거·비교표: `docs/design/combat-lane.md`,
  흐름: `docs/sequences/zone-combat.html`.
- **LB 레인에서 바로 존으로 못 보낸다**: 그 자리는 zoneId를 모른다(봉투에는 clientSessionId만
  있고 zoneId는 `Player`가 들고 있는데 `PlayerRegistry`는 플레이어 레인 전용). 전역
  `sessionId→zoneId` 표를 두면 그게 `Mutexed`가 되어 **락을 없애려다 디스패치 경로에 락을
  만드는 셈**이라 홉 하나를 택했다.
- **네 가지는 흔한 구현과 다르게 갔다**(전부 의도적): ① 위 홉 하나 ② 실패 사유를 전용 태그
  enum이 아니라 `EErrorCode` Combat 대역(400~499)으로 ③ `UnitId`를 `uint64`가 아니라 **`uint32`**
  (플레이어 유닛 id = `clientSessionId`를 자른 값 = `Z2CMoveNotify` 발신자 키라, 폭을 맞춰야
  클라이언트에 변환표가 안 생긴다) ④ 스탯을 `int64`가 아니라 `int32`(틱마다 유닛 수만큼
  나가는 값이라 폭이 그대로 대역폭).
- **틱 타이머(밀린 틱 건너뛰기 포함)는 새로 만들 것이 없었다** — `Timer::RepeatingTimer`가
  절대 시각 예약 + 꼬리 재등록 + `kBypassFactor` + `BypassCount()`를 전부 갖고 있어서
  유닛 정의부터 시작했다.
- **투사체 비행 시간은 서버에 두지 않았다.** 활은 쏘지만 서버는 명중을 즉시 확정하고 화살은
  클라이언트가 그린다 — 비행 상태가 생기면 "날아가는 중에 대상이 죽으면/존을 나가면/시전자가
  끊기면"의 답을 전부 정해야 한다. 붙일 자리는 비워 뒀다(`ApplyHit`이 포인터가 아니라 id를
  받고 매번 다시 찾는다).
- **존 경계를 넘으면 HP가 최대치로 돌아간다.** 유닛은 `Instance` 소유라 존을 옮기면 새로
  만들어지고, 세로 핸드오프에서는 `Player` 자체가 새로 만들어져 이어받을 값이 없다. HP를
  나르려면 World가 전투 스탯을 알아야 하는데 그러면 "World는 전투를 모른다"가 깨진다.
- **클라이언트**: 유닛/HP 바/타겟 링/휘두르기·화살 연출/데미지 숫자/전투 HUD.
  **그림 리소스가 없어 전부 도형**이고 `Ui/CombatArt.cs` 한 곳에 모아, 나중에 아틀라스가
  오면 그 파일 본문만 바뀌게 했다(함수 이름을 프레임 이름과 1:1로 맞춰 둠).
  자동 순회(F2)를 켜면 알아서 싸운다 — 사람이 안 봐도 전투 경로가 도는지 확인하려는 것.
- **`ProtocolClient`에 `attack` 명령** 추가(+ UnitSpawn/AttackResult/UnitDead 출력).
  실측으로 확인한 것: 데미지 22(=(15+10)-3), 몬스터 반격 13, 쿨다운 406, **과잉 데미지가
  남은 HP로 잘림**(22→10), `UnitDead` 정확히 한 번, 8초 뒤 리스폰. WARN/ERROR 0건.
  존 레인 대기 시간 **평균 24µs / 최대 448µs**(틱 주기 100ms 대비 200배 여유).
  자세한 메모와 남은 일은 `docs/local/`의 전투 작업 메모.
...
- **실행 확인**(Debug, 01:33~02:23): 서버 3종 + `Client` 2~3창을 자동 순회로 돌렸다 —
  ...
  1건 제외), 전 레인 `Pending 0`, `ZoneSpace` 61,277콜 평균 대기 22µs.
...
0. **전투 부하 시나리오(StressClient)** — 전투는 **동시성 버그가 나오는 첫 기능**인데 아직
   1~2명으로만 확인했다. 세 가지가 최소 세트다: ① 클라 N개가 **같은 몬스터**를 동시에 때려
   최종 HP가 `maxHp - Σ(데미지)`와 정확히 일치하는가 ② 죽는 순간에 공격이 몰릴 때
   `Z2CUnitDead`가 **정확히 한 번**만 나가는가 ③ 틱 주기를 10배로 올리고 ①②를 반복.
   **③이 "전투를 존 레인에 둔 결정"을 증명하는 테스트다** — 통과 못 하면 어딘가에서 owner를
   `zoneId`가 아닌 값으로 post하고 있는 것이다. 같이 재야 할 것은 존당 인구를 올렸을 때의
   존 레인 큐 대기 시간이고, 그게 곧 샤딩 단위를 갈아탈 시점의 근거가 된다.

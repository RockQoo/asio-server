...
| 9 | **실제 DB 연동** — 로그인이 계정·우편·재화를 실제로 읽는다. **쓰기**(Task→SP 배선)가 남음 | 진행 중 |
...
- **영속화 — 쓰기만** — 로그인은 계정·우편·재화를 **실제로 읽어옵니다**. 반대로 존에서 만든
  변경(Task)은 DB 레인까지 가지만 아직 SP를 호출하지 않아서, 프로세스를 재시작하면 그 사이에
  받은 우편이 소실됩니다 ([DB.html](docs/DB.html))
- **비밀번호 말고는 없는 인증** — 로그인(`C2WLogin`)은 붙었지만 계정이 없으면 그 자리에서
  만듭니다. 세션 토큰도, 중복 로그인 차단도 없습니다 ([login](docs/sequences/login.html))
- **World 캐시가 로그인 시점에서 멈춰 있다** — 존이 올린 UnitOfWork를 캐시에 반영하지 않아서,
  **존에서 새로 만든** 우편만 프로세스를 넘는 핸드오프에서 사라집니다
  ([zone-handoff](docs/sequences/zone-handoff.html))

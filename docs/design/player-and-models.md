...
| 모델 | 보호 | 건드리는 레인 |
|------|------|---------------|
| `wallet_` | 값 그대로(락 없음) | BASIC 레인(owner = `clientSessionId`) 전용 |
| `mailBox_` | `Mutexed` | BASIC 레인 + 만료 스윕(별도 유지보수 타이머) |
| `move_` | `Mutexed` | BASIC 레인(검증·요청 기록) + TICK 레인(틱 적분·경계 판정) |
...
이 객체는 **두 레인이 `shared_ptr`로 함께 들고 있다** — BASIC 레인은 `UnitContainer`
(샤딩된 맵)로, TICK 레인은 `Instance`의 로스터로 도달한다.
...
지금 있는 존은 **BASIC 레인 전용**이라 락이 없다 — 핸드오프도 결국 이 레인으로 오는

...
대상: `Shared/Protocol/Src/StrongId.h`, `Shared/Protocol/Src/Ids.h`
...
## 왜 `Shared/Core`가 아니라 `Shared/Protocol`인가

이 id들은 **Zone/World/클라이언트가 함께 읽고 쓰는 계약**이다. `Shared/Core`는 게임 콘텐츠를
몰라야 하는 정적 라이브러리라 여기 두면 의존 방향이 뒤집힌다 -- `PacketId`/`TaskKind`를
`Shared/Protocol`에 둔 것과 같은 근거다.

부수 효과로 `Core`는 `Shared/Protocol`을 단 한 줄도 include하지 않는다. 그래서
`Network::SessionId`(연결 축)와 `Protocol::PlayerId`(계약 축)가 다른 네임스페이스에 있다 --

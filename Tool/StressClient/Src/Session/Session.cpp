...
namespace Stress
{
    ...
    void Session::HandleEnterZoneNotify(const std::span<const byte> payload)
    {
        MarkProgress();
    ...
    void Session::HandleBroadcastPacket()
    {
    ...
    uint64_t Session::ElapsedUs(const std::chrono::steady_clock::time_point since) noexcept
    {
    ...
    void Session::MarkProgress() noexcept
    {
        lastProgressAtTicks_.store(std::chrono::steady_clock::now().time_since_epoch().count(),
                                    std::memory_order_relaxed);
    }

    std::chrono::steady_clock::time_point Session::LastProgressAt() const noexcept
    {

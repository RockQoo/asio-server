...
namespace Network
{
    ...
    public:
        ...
        // (Connector 주석 참고).
        virtual void OnSessionOpened(const Session::SPtr& session) = 0;

        virtual void OnPacket(const Session::SPtr& session,
                              const Packet::Header& header,
                              const std::span<const byte> payload) = 0;

        virtual void OnClosed(const Session::SPtr& session, const std::error_code& reason) = 0;
    };
}

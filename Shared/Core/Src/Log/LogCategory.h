...
namespace Log
{
    ...
    // 네임스페이스에 정의해서 쓴다 (Server/ZoneServer/Src/Log/LogCategory.h의 Zone::ELogCategory 참고).
    // Log::Entry<TCategory>는 이 타입을 특별 취급하지 않는다 -- 그냥 흔히 쓸 만한 기본
    ...
    // Log::Entry<TCategory>가 ADL로 찾아 호출한다 (LogCategoryType concept 참고).

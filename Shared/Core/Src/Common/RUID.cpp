...
namespace Common
{
    ...
    RUIDGenerator& RUIDGenerator::Instance()
    {
        static RUIDGenerator instance;
        return instance;
    }

    void RUIDGenerator::Initialize(const uint32_t nodeId)
    {
        if (nodeId > kNodeMask)
        {
            // 8비트를 넘으면 조립할 때 밀리초 칸으로 넘쳐서 시각이 미래로 튄다. 그 상태로
            // 발급된 id는 DB에 영구히 남고 나중에 고칠 방법이 없으므로 즉시 드러낸다.
            LOG.Error(ELogCategory::General, "RUID 노드 번호가 8비트를 넘는다")

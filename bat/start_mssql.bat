@echo off
REM 이 프로젝트가 쓰는 SQL Server를 Docker 컨테이너로 띄운다(운영툴 gmtool DB + 게임 asio_game DB).
...
REM
REM 여기서는 컨테이너와 운영툴용 gmtool DB/로그인만 준비한다.
REM   - 운영툴 스키마(Tool\GmTool\Sql\schema.sql): GmTool.Web 이 기동할 때 자동 적용한다
REM     (Database:ApplySchemaOnStartup 설정으로 끌 수 있다).
REM   - 게임 스키마(Sql\schema.sql): bat\setup_game_db.bat 으로 따로 적용한다.
chcp 65001 >nul
setlocal
...
if errorlevel 1 (
    echo [start_mssql] Docker 데몬이 응답하지 않습니다. Docker Desktop 을 먼저 실행하세요.
    pause
    exit /b 1
)
...
if not errorlevel 1 (
    echo [start_mssql] 기존 컨테이너 %NAME% 를 시작합니다.
    docker start %NAME%
    goto :wait
)

echo [start_mssql] 컨테이너 %NAME% 를 새로 만듭니다. 첫 실행은 이미지 다운로드 때문에 몇 분 걸립니다.
...
:wait
echo [start_mssql] 엔진이 준비될 때까지 기다립니다...
...
if %TRIES% GEQ 60 (
    echo [start_mssql] 60회 시도했지만 엔진이 응답하지 않습니다. docker logs %NAME% 를 확인하세요.
    pause
    exit /b 1
)
...
:provision
echo [start_mssql] gmtool 데이터베이스와 로그인을 준비합니다.
...
echo.
echo [start_mssql] 준비 완료.
echo [start_mssql] 접속 정보: 127.0.0.1,1433 / DB gmtool / 계정 gmtool / 비밀번호 %SA_PASSWORD%
echo [start_mssql] 조회 예:   docker exec %NAME% %SQLCMD% -S localhost -U gmtool -P "%SA_PASSWORD%" -C -d gmtool -Q "SELECT name FROM sys.tables"
echo [start_mssql] 종료:      docker stop %NAME%
endlocal

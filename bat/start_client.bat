...
REM
REM C++ 솔루션이 아니라 별도 .NET 솔루션(Tool\Client\Client.slnx)이라 bin\x64 가
...
for %%I in ("%~dp0..\Tool\Client\Client") do set "PROJ=%%~fI"

if not exist "%PROJ%\Client.csproj" (
    echo [start_client] "%PROJ%" 에 Client.csproj 가 없습니다.
    pause
    exit /b 1
)
...
if errorlevel 1 (
    echo [start_client] dotnet SDK 를 찾지 못했습니다. .NET 10 SDK 를 설치하세요.
    pause
    exit /b 1
)
...
for /l %%N in (1,1,%COUNT%) do (
    ...
    start "asio Client %%N (%CONFIG%)" cmd /c "cd /d "%PROJ%" && dotnet run --no-build -c %CONFIG% !ARGS!"
...
echo    dotnet run --project Tool\Client\Client -- 127.0.0.1 9000 http://127.0.0.1:5270
echo.
